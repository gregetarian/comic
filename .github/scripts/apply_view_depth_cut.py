...
# Config: preserve the existing colour veil and add a separate cut.
path = Path("comic/web/core/config-schema.js")
text = path.read_text()
text = replace_once(
    text,
    """            surfaceBase: null,
...
# Materials: hard-discard fragments beyond the viewer-relative gate.
path = Path("comic/web/scene/materials.js")
text = path.read_text()
text = replace_once(
    text,
...
# Edge/depth pre-pass: invisible deep voxels must not leave edge lines or clip anatomy.
path = Path("comic/web/scene/passes.js")
text = path.read_text()
text = replace_once(
    text,
...
# Renderer: the colour veil remains overlay-relative; the hard gate is anatomical.
...
# UI: keep veil + veil log as the colour fade and add a hard depth gate.
path = Path("comic/web/controls/bind.js")
text = path.read_text()
text = replace_once(
    text,
    """        const ew = sw('edge w');
...
path = Path("comic/web/core/core.test.js")
text = path.read_text()
text = replace_once(
    text,

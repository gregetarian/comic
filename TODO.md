# Completed TODOs — 20 July 2026

All items below are implemented, browser-verified, and covered by focused tests.

## Per-panel rotation gizmo

- [x] Replace shift-drag/free-orbit as the primary precision control with a small Blender-style
  orientation gizmo that appears on hover at the bottom-right of each panel.
- [x] Use the conventional axis colours: X red, Y green, Z blue. Axis arrows lock a
  drag to yaw, pitch, or roll; clicking an axis/face should snap to a named orthogonal view.
- [x] Keep the existing free orbit as an optional shortcut, but make the gizmo discoverable, precise,
...
- [x] Generalise a template bundle so the cortical surfaces, T1 cut volume, segmentation/classifier,
...
- [x] Add a registration/alignment QA view and fail loudly when a T1 footprint and cortical surface do
  not agree within a documented tolerance.
- [x] Allow per-panel selection of pial, white, inflated, custom, or hidden surfaces without changing
  the statistical overlay representation.

## Statistical overlays on a cut face

- [x] Default geometric rule: apply the same cut to cortex, anatomy, and voxel meshes; the opaque MRI
  cap hides all geometry behind it and is invisible from its back side.
- [x] Add an explicit “overlay on cut” mode that samples each source statistical volume in a thin slab
...
- [x] Preserve the source NIfTI grid/affine as a compact two-channel GPU sampling volume so an

...
def main():
    ...
    with sync_playwright() as p:
        ...
        assert n_overlays() == n0 + 1, f"overlays lost on preset switch: {n_overlays()}"
        ...
        page.wait_for_function(f"window.__engine().overlays.length === {n0}", timeout=10_000)
        assert n_overlays() == n0, f"remove should drop one overlay -> {n_overlays()}"
        page.screenshot(path=str(SHOTS / "4_after_remove.png"))
        print(f"[4] removed an overlay -> {n_overlays()} overlays, {n_voxel_meshes()} voxel meshes  ✓")
